...
G4ScoringManager* MyRunManager::masterScM = 0;
MyRunManager::masterWorlds_t MyRunManager::masterWorlds = MyRunManager::masterWorlds_t();
MyRunManager* MyRunManager::fMasterRM = 0;
G4int MyRunManager::seedOncePerCommunication = 0;
...
MyRunManager* MyRunManager::GetMasterRunManager()
{
...
G4RunManagerKernel* MyRunManager::GetMasterRunManagerKernel()
{
    return fMasterRM->kernel;
}

G4MTRunManagerKernel* MyRunManager::GetMTMasterRunManagerKernel()
{
    return fMasterRM->MTkernel;
}

MyRunManager::MyRunManager() : G4RunManager(masterRM),
    ...
    nSeedsMax(10000),nSeedsPerEvent(2)
{
    if ( fMasterRM )
    {
        G4Exception("MyRunManager::MyRunManager", "Run0110",FatalException,
                    "Another instance of a MyRunManager already exists.");
    }
    fMasterRM = this;
    ...
    msg << "MyRunManager can only be used in multi-threaded applications.";
    G4Exception("MyRunManager::MyRunManager","Run0111",FatalException,msg);
#endif
    ...
    if(numberOfStaticAllocators>0)
    {
      ...
      G4Exception("MyRunManager::MyRunManager","Run1035",FatalException,msg1);
    }
    ...
    if(env)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          G4Exception("MyRunManager::MyRunManager","Run1039",JustWarning,msg2);
        }
      }
      if(forcedNwokers>0)
      {
...
void MyRunManager::StoreRNGStatus(const G4String& fn )
{
...
void MyRunManager::SetNumberOfThreads(G4int n )
{
    if ( threads.size() != 0 )
    {
      G4ExceptionDescription msg;
      ...
      G4Exception("MyRunManager::SetNumberOfThreads(G4int)",
...
////void MyRunManager::TerminateEventLoop()
////{
////    //Nothing to do
////}
void MyRunManager::ProcessOneEvent(G4int)
{
    //Nothing to do
}
...
void MyRunManager::PrepareCommandsStack() {
...
std::vector<G4String> MyRunManager::GetCommandStack()
{
    G4AutoLock l(&cmdHandlingMutex);
    return uiCmdsForWorkers;
}

void MyRunManager::CreateAndStartWorkers()
{
...
void MyRunManager::InitializeEventLoop(G4int n_event, const char* macroFile, G4int n_select)
{
    TIMEMORY_AUTO_TIMER("");
  ...
  if(!fakeRun)
  {
    ...
    if( eventModuloDef > 0 )
    {
      ...
      if(eventModulo > numberOfEventToBeProcessed/nworkers)
      {
        ...
        G4Exception("MyRunManager::InitializeEventLoop()",
...
void MyRunManager::SetUserInitialization(G4UserWorkerInitialization* userInit)
{
  userWorkerInitialization = userInit;
}

void MyRunManager::SetUserInitialization(G4UserWorkerThreadInitialization* userInit)
{
  userWorkerThreadInitialization = userInit;
}

void MyRunManager::SetUserInitialization(G4VUserActionInitialization* userInit)
{
  userActionInitialization = userInit;
  userActionInitialization->BuildForMaster();
}

void MyRunManager::SetUserInitialization(G4VUserPhysicsList *userPL)
{
    G4RunManager::SetUserInitialization(userPL);
    //Needed for MT, to be moved in kernel 
}

void MyRunManager::SetUserInitialization(G4VUserDetectorConstruction *userDC)
{
    G4RunManager::SetUserInitialization(userDC);
}

void MyRunManager::SetUserAction(G4UserRunAction* userAction)
{
    G4RunManager::SetUserAction(userAction); 
    if(userAction) userAction->SetMaster();
}

void MyRunManager::SetUserAction(G4VUserPrimaryGeneratorAction* /*userAction*/)
{
  G4Exception("MyRunManager::SetUserAction()", "Run0123", FatalException,
    "For multi-threaded version, define G4VUserPrimaryGeneratorAction in G4VUserActionInitialization.");
}

void MyRunManager::SetUserAction(G4UserEventAction* /*userAction*/)
{
  G4Exception("MyRunManager::SetUserAction()", "Run0124", FatalException,
    "For multi-threaded version, define G4UserEventAction in G4VUserActionInitialization.");
}

void MyRunManager::SetUserAction(G4UserStackingAction* /*userAction*/)
{
  G4Exception("MyRunManager::SetUserAction()", "Run0125", FatalException,
    "For multi-threaded version, define G4UserStackingAction in G4VUserActionInitialization.");
}

void MyRunManager::SetUserAction(G4UserTrackingAction* /*userAction*/)
{
  G4Exception("MyRunManager::SetUserAction()", "Run0126", FatalException,
    "For multi-threaded version, define G4UserTrackingAction in G4VUserActionInitialization.");
}

void MyRunManager::SetUserAction(G4UserSteppingAction* /*userAction*/)
{
  G4Exception("MyRunManager::SetUserAction()", "Run0127", FatalException,
    "For multi-threaded version, define G4UserSteppingAction in G4VUserActionInitialization.");
}

void MyRunManager::MergeScores(const G4ScoringManager* localScoringManager)
{
  G4AutoLock l(&scorerMergerMutex);
  if(masterScM) masterScM->Merge(localScoringManager); 
}

void MyRunManager::MergeRun(const G4Run* localRun)
{
  G4AutoLock l(&runMergerMutex);
  if(currentRun) currentRun->Merge(localRun); 
}

G4bool MyRunManager::SetUpAnEvent(G4Event* evt,long& s1,long& s2,long& s3,G4bool reseedRequired)
{
...
G4int MyRunManager::SetUpNEvents(G4Event* evt, G4SeedsQueue* seedsQueue,G4bool reseedRequired)
{
  G4AutoLock l(&setUpEventMutex);
...
void MyRunManager::TerminateWorkers()
{
...
void MyRunManager::AbortRun(G4bool softAbort)
{
...
void MyRunManager::AbortEvent()
{
  // nothing to do in the master thread
}

void MyRunManager::WaitForReadyWorkers() {
...
void MyRunManager::ThisWorkerReady() {
  // beginOfEventLoopBarrier.ThisWorkerReady();
}

void MyRunManager::WaitForEndEventLoopWorkers() {
...
void MyRunManager::ThisWorkerEndEventLoop() {
  // endOfEventLoopBarrier.ThisWorkerReady();
}

void MyRunManager::NewActionRequest(MyRunManager::WorkerActionRequest newRequest) {
...
MyRunManager::WorkerActionRequest MyRunManager::ThisWorkerWaitForNextAction() {
  // nextActionRequestBarrier.ThisWorkerReady();
  return nextActionRequest;
}

void MyRunManager::RequestWorkersProcessCommandsStack() {
  PrepareCommandsStack();
...
void MyRunManager::ThisWorkerProcessCommandsStackDone() {
  processUIBarrier.ThisWorkerReady();
}
void MyRunManager::SetPinAffinity(G4int n)
{
	if ( n == 0 )
	{
		G4Exception("MyRunManager::SetPinAffinity",
